...
export interface OptimizationResult {
  optimizedSpec: CanonicalRequirementSpec;
  additions: string[];
  qualityScore: number;
}
 ...
 */
export function optimizeRequirements(
  spec: CanonicalRequirementSpec,
  defects: string[] = [],
  isFullEnterpriseHardening = false
): OptimizationResult {
  const cloned: CanonicalRequirementSpec = JSON.parse(JSON.stringify(spec));
  ...
  if (
    defects.some((d) => d.includes('compound index') || d.includes('migration')) ||
    !cloned.constraints.some((c) => c.includes('compound index'))
  ) {
    const patch =
      '3NF Relational database schema with compound index on (tenant_id, created_at DESC) and zero loose string IDs.';
  ...
  if (
    defects.some((d) => d.includes('idempotency') || d.includes('replay')) ||
    !cloned.constraints.some((c) => c.includes('HMAC'))
  ) {
    const patch1 =
      'Cryptographic Verification: All state-mutating requests require HMAC-SHA256 signatures with 300s sliding replay protection.';
    const patch2 =
      'Distributed Idempotency: Mandatory Idempotency-Key headers on POST/PUT endpoints using atomic Upstash Redis SETNX with 24h TTL.';
  ...
  if (
    defects.some((d) => d.includes('terminal') || d.includes('rollback')) ||
    !cloned.acceptance_criteria.some((a) => a.includes('terminal'))
  ) {
    const gate1 =
      'Terminal Verification Gate: Run `npm test` and `npx autocannon -c 50 -d 10` ensuring 100% pass and P95 latency < 120ms before phase completion.';
    const gate2 =
      'Automated Rollback Mandate: Any test regression immediately triggers autonomous `git reset --hard HEAD` and root cause incident logging.';
  ...
  if (!cloned.constraints.some((c) => c.includes('Zero loose'))) {
    const patch =
      'Strict Type Safety: Zero loose "any" types; 100% strict TypeScript mode and Zod runtime schema boundaries enforced.';
    cloned.constraints.push(patch);
    additions.push('✨ Enforced strict TypeScript mode and Zod schema boundary validation');
  }

  // 5. Patch Vulnerabilities
  const vulnDefect = defects.find((d) => d.includes('Security Vulnerability Alert'));
  if (vulnDefect) {
    cloned.constraints.push(
      'Dependency Security Mandate: Pin safe package versions; upgrade vulnerable dependencies to eliminate CVEs.'
    );
    additions.push('🛡️ Vulnerability Patch: Mandated secure non-vulnerable dependency bounds');
  }

  // 6. Level 2 Enterprise Observability & SOC2
  if (isFullEnterpriseHardening) {
    ...
    const obs2 = 'Immutable SOC2 audit trail logging with correlation IDs (trace_id, span_id, tenant_id) in structured JSON.';
