...
export class AntigravityAdapter extends BaseAdapter {
  ...
  adapt(spec: CanonicalRequirementSpec, _options: Record<string, unknown> = {}): AdaptedOutputs {
    const tech = spec.technical_stack || { frontend: [], backend: [], database: [], infra: [] };
    const frontend = (tech.frontend || []).join(', ') || 'React 19, Tailwind CSS';
    const backend = (tech.backend || []).join(', ') || 'Node.js 24 LTS, Fastify 5.x, TypeScript';
    const database = (tech.database || []).join(', ') || 'Supabase PostgreSQL, Prisma ORM';
...
<phase_5_final_verification_and_signoff>
- Command: \`npm run test:all\`
- Gate: 100% test pass across all unit, integration, and security test gates.
</phase_5_final_verification_and_signoff>`;
