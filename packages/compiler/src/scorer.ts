...
export interface ScoreResult {
  ...
  status: 'PASS' | 'NEEDS_EXPANSION' | 'FAIL';
  suggestions: string[];
}
...
export function scorePrompt(
  ...
  const hasStack =
    ...
    Object.values(spec.technical_stack).some((arr) => Array.isArray(arr) && arr.length > 0);
  let context = hasStack ? 10 : 6;

  // 6. Model & Agent Dialect Fit (max 10)
  let modelFit = ['antigravity', 'cursor', 'claude', 'v0'].includes(targetAgent.toLowerCase()) ? 10 : 8;

  // 7. Edge Cases & Adversarial Invariants (max 10)
  const hasSecurity =
    text.toLowerCase().includes('hmac') ||
    text.toLowerCase().includes('idempotency') ||
    text.toLowerCase().includes('security') ||
    spec.constraints?.some(
      (c) => c.toLowerCase().includes('replay') || c.toLowerCase().includes('idempotency')
    );
  ...
  const totalScore = Math.min(
    100,
    Math.round(clarity + completeness + constraints + gating + context + modelFit + edgeDefenses)
  );

  let status: 'PASS' | 'NEEDS_EXPANSION' | 'FAIL' = 'FAIL';
  if (totalScore >= 90) {
    status = 'PASS';
